...
def lambda_handler(event, context):
    ...
    for i in response['Items']:
        ...
        # VULNERABLE CODE (before fix) - token field exposed to client
        # item = {"order-id": i['orderId'], "date": i['paymentTS'], "total": i['totalAmount'], "status": human_status[status], "token": i["confirmationToken"] }

        # FIX: removed "token" field - confirmationToken is internal only
        item = {"order-id": i['orderId'], "date": i['paymentTS'], "total": i['totalAmount'], "status": human_status[status]}
        orders.append(item)
